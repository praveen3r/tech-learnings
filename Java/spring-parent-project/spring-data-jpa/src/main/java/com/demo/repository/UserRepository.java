...
public interface UserRepository extends JpaRepository<User, BigInteger> {
	...
	List<User> findByUserNameIsNotNull(String userName);
	...
	List<User> findByIdStartingWith(BigInteger fromId, BigInteger toId);
	List<User> findByIdEndingWith(BigInteger fromId, BigInteger toId);
	...
	@Procedure(name="userAuthenticationAdmin")
	Integer authenticateAdmin(@Param("p_corpId") String corpId, @Param("p_username") String username, @Param("p_password") String hashedCredential);
	
	@Procedure(name="userAuthentication")
	Integer authenticatePortal(@Param("p_corpId") String corpId, @Param("p_username") String username, @Param("p_password") String hashedCredential, 
			@Param("p_appid") String applAccess);*/
	
	/*@Query(
		value = "SELECT u.userId, u.bankId FROM User u",
		 countQuery = "select count(u.userId) from User u")
	Page<User> getUserDetails1(Pageable pageable);
	
	@Query(
	value = "SELECT new com.demo.UserVO(u.userId, u.bankId) FROM User u",
	 countQuery = "select count(u.userId) from User u")
	Page<UserVO> getUserDetails1(Pageable pageable);
	
	@Query("SELECT new com.demo.UserVO(u.userId, u.bankId) FROM User u WHERE u.id = ?1 ")
	Page<UserVO> getUserDetails2(Long userId, Pageable pageable);*/
