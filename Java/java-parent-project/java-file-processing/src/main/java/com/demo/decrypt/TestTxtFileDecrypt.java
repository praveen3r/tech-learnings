...
public class TestTxtFileDecrypt {

	public static void main(String[] args) throws Exception{
		String key = "This is a secret";
		TestTxtFileEncrypt testFileEncrypt = new TestTxtFileEncrypt();
		String path = testFileEncrypt.getClass().getClassLoader().getResource("test-encrypted.txt").getPath();
		String configPath = URLDecoder.decode(path, "UTF-8");
		File file = new File(configPath);
		String projectFolder = configPath.split("target")[0];
	    File encryptedFile = new File(projectFolder + "src/main/resources/" + "test-decrypted.txt");
			
		try {
			fileReader(Cipher.DECRYPT_MODE,key,file,encryptedFile);
